module AgencyPeopleHelper
  ...
  def job_seekers_without_job_developer
    # Returns a sorted (last_name) array of job seekers that are not assigned
    # to any job developer
    job_seekers_without_agency_person_in_role(:JD).
                  sort {|js1, js2| js1.last_name <=> js2.last_name }
  end

  def job_seekers_without_case_manager
    # Returns a sorted (last_name) array of job seekers that are not assigned
    # to any case manager
    job_seekers_without_agency_person_in_role(:CM).
                  sort {|js1, js2| js1.last_name <=> js2.last_name }
  end
